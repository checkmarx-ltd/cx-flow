...
| preserve-project-name | false      | No | Yes | Yes       | When **False**: The project name for the scanner will be the repository name after normalization. Legal characters are: 'a-z', 'A-Z', '0-9', "-", "_", ".". All other characters will be replaced in the normalization process with "-".
When **True**: The project name for the scanner will be the same as the repository name without normalization. For attention: Not all scanners allow project names with invalid characters. |
| **checkmarx**     |                |    |     |           |                                                                           |
