...
| preserve-project-name | false      | No | Yes | Yes       | When **False**: The project name will be the repository name after normalization. Legal characters are: `a-z`, `A-Z`, `0-9`, `-`, `_`, `.`. All other characters, include spaces will be replaced in the normalization process with "-". <br/> When **True**: The project name will be the same as the repository name without normalization. For attention: Not all scanners allow project names with invalid characters. |
| **checkmarx**     |                |    |     |           |                                                                           |
