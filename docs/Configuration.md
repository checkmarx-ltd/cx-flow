...
| `email-notifications`                  |                       | No                                  |         | Yes (Scan only)   | A map containing any or all of the following keys: `after-scan`, `before-scan`, `failed-scan`. The value of each key is a list of email addresses to which a notification should be sent in the case of the relevant event.                                                                                                                                                                                                                                                             |
...
| `restrict-results-to-branch`           | false                 | No                                  | Yes     | Yes (Scan only)   | If set to `true`, this option ensures that only vulnerabilities found on the current branch are reported during a scan of a branched project. It requires the setting `cx-branch: true` to be enabled. Additionally, the CxSAST user must have a role with the “API” permission, as the OData API is needed. Ensure that the `client-id` is set to `resource_owner_sast_client` and the `scope` includes both access_control_api and sast_api.                                          |
| `project-summary`                      | true                  | No                                  | Yes     | Yes (Scan only)   | If set to `false`, When scanning with `restrict-results-to-branch` functionality. project summary results in PR with branch summary can be disabled.                                                                                                                                                                                                                                                                                                                                    |
