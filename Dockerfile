...
RUN apk add bash
#RUN apk add curl
RUN wget --header="https://sca-downloads.s3.amazonaws.com" "https://sca-downloads.s3.amazonaws.com/cli/latest/ScaResolver-musl64.tar.gz" -O 'ScaResolver.tar.gz' && tar -xvzf ScaResolver.tar.gz && mv ScaResolver Configuration.yml /app && rm ScaResolver.tar.gz
